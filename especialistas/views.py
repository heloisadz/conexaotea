...
def createespecialista(request):
    if request.user.groups.filter(name='administrador').exists():
        if request.method == 'POST':
            ...
            form = EspecialistaForm()
        return render(request, 'especialistas/createespecialista.html', {'form': form})
    else:
...
def updateespecialista(request, id_especialista):
    if request.user.groups.filter(name='administrador').exists():
        ...
        return render(request, 'especialistas/updateespecialista.html', {'form': form, 'id_especialista': id_especialista})
    else:
        #raise Http404("Opa! Aqui não.")
        return render(request, 'error.html', {'message': 'Você não tem permissão para acessar esta página.'})
...
def confirmDelete(request, id_especialista):
    if request.user.groups.filter(name='administrador').exists():
        especialista = get_object_or_404(Especialista, pk=id_especialista)
        return render(request, 'especialistas/confirmdeleteespecialista.html', {'especialista': especialista})
    else:
        return render(request, 'error.html', {'message': 'Você não tem permissão para acessar esta página.'})
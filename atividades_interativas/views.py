...
def createatividade(request):
    if request.user.groups.filter(name='administrador').exists():
        if request.method == 'POST':
            ...
            form = AtividadesForm()
        return render(request, 'atividades/createatividade.html', {'form': form})
    else:
...
def atividades(request):
    ...
    return render(request, 'atividades/atividades.html', {'atividades': atividades})

@login_required
def readatividades(request):
    atividades = Atividades.objects.all()
    return render(request, 'atividades/readatividades.html', {'atividades': atividades})
...
def updateatividade(request, id_atividade):
    if request.user.groups.filter(name='administrador').exists():
        ...
        return render(request, 'atividades/updateatividade.html', {'form': form, 'id_atividade': id_atividade})
    else:
        return render(request, 'error.html', {'message': 'Você não tem permissão para acessar esta página.'})
...
def confirmDelete(request, id_atividade):
    if request.user.groups.filter(name='administrador').exists():
        atividade = get_object_or_404(Atividades, pk=id_atividade)
        return render(request, 'atividades/confirmdeleteatividade.html', {'atividades': atividade})
    else:
        return render(request, 'error.html', {'message': 'Você não tem permissão para acessar esta página.'})